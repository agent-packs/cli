...
func runInstall(registry, defaultTarget string, args []string) error {
	...
	*agent = agentpacks.NormalizeAgent(*agent)
	...
	if *only != "all" && *only != "skills" && *only != "plugins" && *only != "memory" && *only != "settings" && *only != "commands" && *only != "hooks" {
		return fmt.Errorf("invalid --only %q: expected all, skills, plugins, memory, settings, commands, or hooks", *only)
	}
...
_agent_packs() {
    ...
    case "$prev" in
        ...
        --on-conflict)
            ...
            return ;;
        --only)
            COMPREPLY=($(compgen -W "all skills plugins memory settings commands hooks" -- "$cur"))
            return ;;
    esac
    ...
    case $state in
        ...
        args)
            ...
            _arguments \
                ...
                '--only=[capability filter]:filter:(all skills plugins memory settings commands hooks)' \
...
complete -f -c agent-packs -l only         -a "all skills plugins memory settings commands hooks"  -d 'Capability filter'
